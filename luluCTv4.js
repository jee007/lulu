(async function() {
    ...
    const INTERVAL_WAIT = 150 * 60 * 1000;
    const startTime = Date.now();
    let cycleCount = 0;
    let lastMatrixData = { quick: [], schedule: [] };
    let forceRefresh = false; // Flag for manual trigger
    ...
    const updateUI = (status, nextRun = null) => {
        ui.innerHTML = `
            ...
            <button id="refresh-now-btn" style="width:100%; background:#2196F3; color:white; border:none; padding:8px; cursor:pointer; font-weight:bold; border-radius:4px;">🔄 Sync Now</button>
        ...
        document.getElementById('refresh-now-btn').onclick = () => { forceRefresh = true; };
    };
    ...
    while (Date.now() - startTime < TOTAL_RUN_TIME) {
        ...
        forceRefresh = false; // Reset flag before wait
        while (Date.now() < nextTime && !forceRefresh) {
            updateUI("⏳ Standby", new Date(nextTime));
            await new Promise(r => setTimeout(r, 1000)); // Check manual trigger every second
        }
    }
})();
