(async function() {
    ...
    const INTERVAL_WAIT = 150 * 60 * 1000; // 2.5 hours as per your code
    const startTime = Date.now();
    let cycleCount = 0;
    let lastMatrixData = { quick: [], schedule: [] };
    let forceRefresh = false; 
    let isWorking = false; // New state tracker
    ...
    const updateUI = (status, nextRun = null) => {
        ...
        const btnColor = isWorking ? "#FF9800" : "#2196F3"; // Orange if working, Blue if idle
